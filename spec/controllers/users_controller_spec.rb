  ...
  describe "POST create" do
    context "with valid user input" do
      ...
      context "with valid card info" do
        ...
        it "creates the user" do
          post :create, user: {full_name: 'Jon Doe', password: 'sesame', email: "jon@example.com"}
          User.first.full_name.should == "Jon Doe"
        end

        it "sends the welcome email" do
          post :create, user: {full_name: 'Jon Doe', password: 'sesame', email: "jon@example.com"}
          ActionMailer::Base.deliveries.should_not be_empty
          ActionMailer::Base.deliveries.first.to.should == ['jon@example.com']
        end
    ...
    context "with invalid user inputs" do
      context "with valid credit card info" do
        ...
        it "does not create the user" do
          post :create, user: {password: 'sesame', email: "jon@example.com"}
          User.count.should == 0
        end
    ...
    context "with invitation token" do
      ...
      context "with valid card info" do
        before do
          charge = double('charge', successful?: true)
          StripeWrapper::Charge.stub(:create).and_return(charge)
          post :create, {user: {full_name: 'Jon Doe', password: 'sesame', email: "jon@example.com"}, token: invitation.token}
        end

        it "the signed up user and the inviter follow each other" do
          jon = User.where(email: 'jon@example.com').first
          bob = invitation.inviter
          jon.follows?(bob).should be_true
          bob.follows?(jon).should be_true
        end
      ...
      context "with invalid card info" do
        ...
        it "does not sign up the user" do
          User.where(email: "jon@example.com").count.should == 0
        end
